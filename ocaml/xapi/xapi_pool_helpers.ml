...
let valid_operations ~__context record _ref' =
	...
	if List.mem `ha_enable current_ops
		then begin
			set_errors Api_errors.ha_enable_in_progress
				[ "pool"; _ref; pool_operation_to_string `ha_enable ] [ `ha_enable ];
			set_errors Api_errors.ha_enable_in_progress
				[ "pool"; _ref; pool_operation_to_string `ha_enable ] [ `ha_disable ]
		end;
	(* HA enable or disable cannot run if HA disable is in progress *)
	if List.mem `ha_disable current_ops
		then begin
			set_errors Api_errors.ha_disable_in_progress
				[ "pool"; _ref; pool_operation_to_string `ha_enable ] [ `ha_enable ];
			set_errors Api_errors.ha_disable_in_progress
				[ "pool"; _ref; pool_operation_to_string `ha_enable ] [ `ha_disable ]
		end;
	...
	if ha_enabled then
		set_errors Api_errors.ha_is_enabled [ "pool"; _ref; "ha_enabled" ] [ `ha_enable ]
	else
		set_errors Api_errors.ha_is_enabled [ "pool"; _ref; "ha_disabled" ] [ `ha_disable ];
