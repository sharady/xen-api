...
let valid_operations ~__context record _ref' =
    ...
    (* HA enable and disable cannot run concurrently *)
    if List.mem `ha_enable current_ops
        then set_errors Api_errors.ha_enable_in_progress
            [ "pool"; _ref; pool_operation_to_string `ha_enable ] [ `ha_disable ];
    if List.mem `ha_disable current_ops
        then set_errors Api_errors.ha_disable_in_progress
            [ "pool"; _ref; pool_operation_to_string `ha_disable ] [ `ha_enable ];
    table
