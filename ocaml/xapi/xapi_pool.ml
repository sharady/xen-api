...
let pre_join_checks ~__context ~rpc ~session_id ~force =
  ...
  let allow_management_interface_on_vlan () =
    ...
    | [vlan] ->
      (try
        if Db.PIF.get_VLAN ~__context ~self:(
            Xapi_host.get_management_interface ~__context ~host:(Helpers.get_localhost ~__context)) =
          Client.PIF.get_VLAN rpc session_id (
            Client.Host.get_management_interface rpc session_id (get_master ~rpc ~session_id)) then true else false
       with e -> error "Caught %s: while checking joining host and the pool have management interface on same vlan" (ExnHelper.string_of_exn e);
         false)
    | _ -> false
  in

  let assert_only_physical_or_vlan_pifs () =
  ...
  assert_only_physical_or_vlan_pifs ();
