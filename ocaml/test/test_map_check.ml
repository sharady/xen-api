...
module AssertAllKeys = Generic.Make(struct
	module Io = struct
		...
		let string_of_input_t (ty, ks, value, db) =
			Printf.sprintf "%s, %s, %s, %s" ty "" "" ""
			(*Test_printers.(tuple4 string
			(pair string (list (pair string (list (pair string (pair string string))))))
			(assoc_list string string)
			(assoc_list string string))*)
		let string_of_output_t = Test_printers.(assoc_list string string)
	end
...
module AssertKeys = Generic.Make(struct
	module Io = struct
		...
		let string_of_input_t (ty, ks, value, db) = Printf.sprintf "%s, %s, %s, %s" ty "" "" ""
	...
	let tests = [
		...
		("", ("", ["", ["min",(String,"")]]), ["hour","0";"min","30"], ["min", "0"]),
		Either.Left (Api_errors.(Server_error (invalid_value, [":hour"; "0"])));
	]
